...
void ThumbnailView::updateThumbnails() {
    ...
    selectButton->setEnabled(!images[indices[1]]->placeholder());
...
void ThumbnailView::selectImage(){
    qDebug() << images[current]->fullPath();
    emit newImage(images[current]->pixmap(), images[current]->filename()+".dng");
}
